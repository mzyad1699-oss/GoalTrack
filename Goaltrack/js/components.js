import { DAY_NAMES_AR, formatArabicDate, ARABIC_MONTHS, escapeHtml } from './utils.js';
...
export function renderSearchResult(result) {
  ...
  return `
    <button class="search-result" data-date="${result.date || ''}">
      ...
      <span class="search-result__snippet">${escapeHtml(result.snippet)}</span>
    ...
    <div class="task-card ${isCompleted ? 'task-card--done' : ''}" data-goal-id="${goal.id}" style="--goal-color:${goal.color}">
      <div class="task-card__main">
        ...
        <div class="task-card__info">
          <span class="task-card__name">${escapeHtml(goal.name)}</span>
        ...
        <input type="time" class="task-time js-task-time" data-goal-id="${goal.id}" value="${escapeHtml(time)}" />
      </div>
      <textarea
...
export function renderReportCard(report, dateFormatter) {
  ...
  const bestWorstHtml = `
    ...
    <div class="report-row">
      ${report.bestGoal && report.bestGoal.total > 0 ? `<span>🏆 أفضل هدف: ${escapeHtml(report.bestGoal.name)}</span>` : ''}
      ${report.worstGoal && report.worstGoal.total > 0 ? `<span>🔻 أقل هدف: ${escapeHtml(report.worstGoal.name)}</span>` : ''}
    </div>
  `;
    ...
    .map((g) => {
      ...
      return `
        <div class="year-progress-row">
          <div class="year-progress-row__label"><span>${escapeHtml(g.name)}</span><span>${pct}%</span></div>
...
export function renderTemplateCard(template, isMine) {
  const goalsPreview = (template.goals || []).slice(0, 5).map((g) => escapeHtml(g.name)).join(' • ');
  return `
    <div class="template-card">
      <div class="template-card__title">${escapeHtml(template.title)}</div>
      <div class="template-card__desc">${escapeHtml(template.description || '')}</div>
...
export function renderAccountDropdownList(currentUser, otherAccounts) {
  ...
  const currentItem = `
    <div class="account-dropdown-item account-dropdown-item--current">
      <div class="account-dropdown-item__avatar">${escapeHtml(currentInitial)}</div>
      <span class="account-dropdown-item__email">${escapeHtml(currentUser.email)}</span>
    ...
    .map((a) => {
      ...
      return `
        <button class="account-dropdown-item js-dropdown-switch" data-user-id="${a.userId}">
          <div class="account-dropdown-item__avatar">${escapeHtml(initial)}</div>
          <span class="account-dropdown-item__email">${escapeHtml(a.email)}</span>
        </button>
      `;
    })
    .join('');
...
export function renderAccountRow(account) {
  ...
  return `
    <div class="account-row">
      <div class="account-row__avatar">${escapeHtml(initial)}</div>
      <div class="account-row__info">
        <span class="account-row__name">${escapeHtml(label)}</span>
        <span class="account-row__email">${escapeHtml(account.email)}</span>
      </div>
...
export function renderGoalManagerRow(goal) {
  return `
    <div class="goal-row" data-goal-id="${goal.id}">
      ...
      <input type="text" class="goal-row__name js-goal-name" data-goal-id="${goal.id}" value="${escapeHtml(goal.name)}" />
